...
float voronoi(vec2 uv, inout vec2 n_point, inout vec2 s_n_point, inout float s_dist) {
  ...
  for(float j = -1.; j < 2.; j++) {
    for(float i = -1.; i < 2.; i++) {
      ...
      rand = hash2(grid_test_id + 1000.)*.4;
...
vec4 renderPass(vec2 uv, vec2 uvoffset) {
  ...
  vec3 lightPos = vec3(mouse.x, mouse.y, -3.) + vec3(movement, 0.);
...
void main() {
  ...
  mouse = (u_mouse2.xy - 0.5 * u_resolution.xy) / min(u_resolution.y, u_resolution.x);
...
function init() {
    ...
    uniforms = {
        ...
        u_mouse2: { type: "v2", value: new THREE.Vector2() }
    };
    ...
    document.addEventListener('pointermove', (e) => {
        ...
        uniforms.u_mouse2.value.x = e.pageX;
        uniforms.u_mouse2.value.y = e.pageY;
